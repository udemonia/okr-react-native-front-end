...
const OKRsReducer = (state, action) => {
    switch ( action.type ) {
            ...
            return [...state, 
        {
                ...
                "name": "TEST OBJECTIVE ON USE STATE",
...
export const OKRsProvider = ({ children }) => {
    ...
    const addOKR = () => {
        dispatch({ type: addOKR })
    }

    console.log('\nThis is the Start of the new Array\n')
    console.log(objectivesArray)

    return <OKRsContext.Provider value={{ objectivesArray, addOKR }}>
        {children}
...
//     const [ objectivesArray, setObjectivesArray ] = useState(OKRs)
